...
QTextStream &operator>>(QTextStream &in, Qcallog &log)
{
    ...
    log.date = QDate::fromString(datestr, dateformat);
    log.date = log.date.addYears(100);
    log.time = QTime::fromString(timestr, timeformat);
    in >> log.linelen;
    in >> log.callen;
    in >> log.relreason;
    in.skipWhiteSpace();
...
Qlogdb operator << (Qlogdb &logdb, Qcallog &log)
{
    ...
    QSqlQuery query;
    query.prepare("INSERT INTO  logbase (intype, ininc1, ininc2, ininc3, innum, inanum, outtype, outinc1, outinc2, outinc3, outnum, outanum, date, time, linelen, callen, relreason) VALUES (:intype, :ininc1, :ininc2, :ininc3, :innum, :inanum, :outtype, :outinc1, :outinc2, :outinc3, :outnum, :outanum, :date, :time, :linelen, :callen, :relreason)");
